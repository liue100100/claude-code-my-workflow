...
The 11 specialist subagents, what each is for, the model tier it runs at ([`model-routing.md`](../rules/model-routing.md)), and which skill dispatches it. This makes the fleet legible — when a fan-out skill picks a lens, this is the roster it picks from. Reviewers are **read-only** (they report `FINDING`s per [`orchestration-schemas.md`](orchestration-schemas.md)); only the fixer edits files.

> Seven lecture-slide-specific agents (`domain-reviewer`, `quarto-critic`, `tikz-reviewer`, `slide-auditor`, `pedagogy-reviewer`, `quarto-fixer`, `beamer-translator`) are archived under `.claude/agents/_archived-lecture/` — not part of the active fleet for this paper project.
...
| `verifier` | End-to-end LaTeX-compile / R-run / Python-run verification gate | read-only | no | `/commit` |

## Review / critique tier — Sonnet 4.6, effort: high

| Agent | Role | Read/Write | Dispatched by |
|---|---|---|---|
| `r-reviewer` | R code quality, reproducibility, idioms | read-only | `/review-r`, `/data-analysis` |
| `r-package-reviewer` | R package CRAN-readiness (DESCRIPTION/NAMESPACE/roxygen/testthat/policy) | read-only | `/r-package-check` |
| `proofreader` | Grammar, typos, overflow, terminology | read-only | `/proofread`, `/seven-pass-review` (prose lens) |
